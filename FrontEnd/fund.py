...
def index():
    with get_db().cursor() as cursor:
        ...
        for fund in funds:
            cursor.execute(
                "SELECT picture FROM Images WHERE FundName = %s LIMIT 1", (fund[0],)
            )
...
def fundraisers(fund_name):
    if request.method == "POST":
        ...
        get_db().cursor().execute(
            "UPDATE Funds SET FundRaised = FundRaised+%s WHERE FundName = %s",
            (amount, title),
        )
        get_db().commit()
        if g.user:
            get_db().cursor().execute(
                "INSERT INTO Donations (FundName, UserID, DonoAmount, DonoTime, DonoComment) VALUES (%s,%s,%s,%s,%s)",
                (title, g.user, amount, datetime.datetime.now(), comment),
            )
            get_db().commit()
        else:
            get_db().cursor().execute(
                "INSERT INTO Donations (FundName, UserID, DonoAmount, DonoTime, DonoComment) VALUES (%s,%s,%s,%s,%s)",
                (title, "Anonymise", amount, datetime.datetime.now(), comment),
            )
            get_db().commit()
        return redirect(url_for("fund.fundraisers", fund_name=fund_name))
    with get_db().cursor() as cursor:
        ...
        cursor.execute(
            "SELECT UserID,DonoAmount,DonoComment FROM Donations WHERE FundName = %s",
            (fund_name),
        )
        comments = cursor.fetchall()

    return render_template(
        "fundraisers.html", fund=fund, images=images, comments=comments
    )
...
def form():
    if request.method == "GET":
        ...
        with get_db().cursor() as cursor:
            ...
            try:
                cursor.execute(
                    "INSERT INTO Funds (FundName, FundEndDate, FundDesc, FundGoal, FundRaised, FundStart, FundTags) VALUES (%s, %s, %s, %s, 0, %s, %s)",
                    (title, end_date, description, goal, start_date, tags),
                )
                cursor.execute(
                    "INSERT INTO UserFundLink (UserId,FundName) VALUES (%s,%s)",
                    (user_id, title),
                )
            except:
            ...
            for image in upload:
                ...
                with open(image.filename, "rb") as f:
                    binaryData = f.read()
                cursor.execute(
                    "INSERT INTO Images (FundName, picture) VALUES (%s, %s)",
                    (title, binaryData),
                )
                get_db().commit()
